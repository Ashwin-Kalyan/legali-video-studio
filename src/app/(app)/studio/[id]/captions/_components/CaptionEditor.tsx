...
export function CaptionEditor({ project }: { project: VideoProject }) {
  ...
  function applyLang(code: string) {
    ...
    setAnno(
      `Translation: POST /api/v1/projects/${project.id}/captions/translate — {target_language:"${code}"}. Claude API translates the full transcript preserving timing. Bilingual export (EN + ${code.toUpperCase()}) available for SRT.`,
    );
  }
  ...
  function regenerate() {
    setAnno(
      `Regenerate: POST /api/v1/projects/${project.id}/captions/generate — re-runs the Claude caption prompt with ${brand.brandName} voice context. Replaces existing caption_json. Safety check auto-runs before saving.`,
    );
  }
  ...
  function runSafety() {
    setSafetyOpen(true);
    setAnno(
      `Safety check: POST /api/v1/projects/${project.id}/captions/safety-check → Claude API trauma-informed pass. Returns {passed:true, flags:[], revised_text:null}. Required before any Lea brand export or schedule action.`,
    );
  }
        ...
        <div className="overflow-hidden rounded-2xl border border-rule bg-surface shadow-card">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-rule bg-surface-2 px-4 py-3">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              <ToolbarButton onClick={regenerate} icon={<IconRefresh size={14} stroke={1.75} />}>
                Regenerate
              </ToolbarButton>
              <ToolbarButton onClick={runSafety} icon={<IconShield size={14} stroke={1.75} />}>
                Safety check
              </ToolbarButton>
              <Link
                ...
                <div className="grid grid-cols-3 gap-2">
                  {LANGUAGES.map((l) => (
                    <button
                      ...
                      className={cn(
                        "rounded-md border px-2 py-1.5 text-[0.7rem] font-medium transition-colors",
                        lang === l.code
